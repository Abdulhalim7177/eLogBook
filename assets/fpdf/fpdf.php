...
function GetStringWidth($s)
{
	...
	for($i=0;$i<$l;$i++) {
		if(!isset($cw[$s[$i]])) {
			// If character not found, use default width
			$w += $cw['?'] ?? 600; // Default width if '?' not defined
			continue;
		}
		$w += $cw[$s[$i]];
	}
...
function SetFont($family, $style='', $size=0)
{
	...
	if(!isset($this->fonts[$fontkey]))
	{
		...
		if($family=='arial')
			$family = 'helvetica';
		if(!isset($this->CoreFonts) || !is_array($this->CoreFonts)) {
			$this->CoreFonts = array('courier', 'helvetica', 'times', 'symbol', 'zapfdingbats');
		}
		...
		else
		{
			...
			$this->AddFont($family,$style);
			if(!isset($this->fonts[$fontkey])) {
				// If still not loaded, use helvetica as fallback
				$family = 'helvetica';
				$fontkey = $family.$style;
				if(!isset($this->fonts[$fontkey])) {
					$this->AddFont($family,$style);
