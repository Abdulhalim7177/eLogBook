<?php 
	...
	$getpost = "SELECT * FROM logbook 
				INNER JOIN reglist ON logbook.logbookMat = reglist.matno 
				INNER JOIN siwespost ON reglist.matno = siwespost.siwesMat 
				WHERE logbookMat = '$matno' 
				ORDER BY logbookId DESC";
	$thepost = mysqli_query($connection, $getpost);

	$getsiwes = "SELECT * FROM logbook 
				 INNER JOIN reglist ON logbook.logbookMat = reglist.matno 
				 INNER JOIN siwespost ON reglist.matno = siwespost.siwesMat 
				 WHERE siwesMat = '$matno' 
				 ORDER BY siwesPostId DESC LIMIT 1";
			...
			<div class="row">
				<div class="col-sm-8 page-content">
					<div class="white-container mb0">
						<a href="student.php" class="btn btn-default">View all Students</a>
						<?php while($siwesrow = mysqli_fetch_array($siwespost)){ ?>
							...
							<p>
								<strong>Organisational Supervisor:</strong> <?php echo $siwesrow['siwesSupervisor'];?><br>
								<strong>Supervisor No:</strong> <?php echo $siwesrow['siwesSupervisorNo'];?>
							</p>
						<?php } ?>
